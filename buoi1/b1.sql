...
CREATE VIEW demo as 
select r.region_name, c.country_name, l.street_address,l.postal_code, l.city,l.state_province  from regions as r
...
DELIMITER $$
create trigger `Insert_jobs`
before insert on jobs
for each row
begin
	if(new.min_salary < 0 or new.max_salary < 0) then 
		signal sqlstate '45000'
        set MESSAGE_TEXT = ' min_salary or max_salary is error';
	end if;
end;$$


INSERT INTO `qlds`.`jobs` (`job_title`, `min_salary`, `max_salary`) VALUES ('TestTrigger', -1, -1);
...
begin
	if(new.min_salary < 0 or new.max_salary < 0) then 
		signal sqlstate '45000'
        set MESSAGE_TEXT = ' min_salary or max_salary value is error';
	end if;
end;$$
UPDATE `qlds`.`jobs` SET `max_salary` = '-10' WHERE (`job_id` = '2');
...
DO
	INSERT INTO `qlds`.`regions` ( `region_name`) VALUES ('Chau my') ;
...
-- tạo 1 trigger khi insert vào bảng employees nếu salary nhập vào không nằm trong khoảng min max của bảng jobs thì thông báo lỗi
delimiter $$
...
begin
   ...
   set min = (select min_salary from jobs where job_id = new.job_id);
   set max = (select max_salary from jobs where job_id = new.job_id);
   if new.salary < min or new.salary > max then
   signal sqlstate '45000'
